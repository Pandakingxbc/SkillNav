...
public:
  ...
  unique_ptr<MultiValueMapManager> multi_valuemap_manager_;  ///< Multi-ValueMap manager for prompt-guided exploration
  unique_ptr<MemoryAgent> memory_agent_;  ///< Memory Agent for candidate target management and FP tracking
  unique_ptr<ExplorationAgent> exploration_agent_;  ///< Exploration Agent: SR/IG fusion-weight controller

  /// Non-owning accessor for AgentCoordinator. ExplorationManager remains the
  /// owner of MemoryAgent today; the Coordinator only views it. Returns null
  /// if MemoryAgent was not constructed (skip-config or pre-init).
  MemoryAgent* getMemoryAgent() { return memory_agent_.get(); }
...
private:
  ...
  /**
   * @brief HFTN (Hybrid Frontier-Topology) policy — design memo C1.
   *
   * Two-level decision:
   *   1. Score each Voronoi node that has at least one anchored frontier
   *      (see VoronoiTopology::computeFrontierAnchors). Pick the best.
   *   2. Pick the most reachable anchored frontier within that node's set.
   *   3. If the best node has no reachable anchored frontier, walk down the
   *      sorted node list. If no node yields any reachable frontier, fall
   *      back to legacy per-frontier semantic scoring on the orphan
   *      frontiers (then on all frontiers as ultimate fallback).
   *
   * Inputs: `frontiers` are the current-cycle frontier centroids (same
   * format as the other policies); HFTN re-fetches the FrontierMap2D
   * arrays internally so it can match anchored ids back to coordinates.
   */
  void findHFTNPolicy(Vector2d cur_pos, vector<Vector2d> frontiers, Vector2d& next_best_pos,
      vector<Vector2d>& next_best_path);
