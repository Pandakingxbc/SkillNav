...
MapROS::~MapROS() {
  // Minimal destructor — only the lifetime guards (alive_token_ + mvm_=null)
  // remain, because they prevent crashes from queued message_filters callbacks
  // firing on a destroyed `this`. Everything else (timer stop, clearVis,
  // sleep) is removed to match ApexNav's default-destruction pattern.
  alive_token_.reset();
  mvm_ = nullptr;
}

void MapROS::clearVisualization() {
  ROS_INFO("[MapROS] Clearing all map visualization...");

  // Create empty point cloud - use "world" as fallback frame_id
  sensor_msgs::PointCloud2 empty_cloud;
  empty_cloud.header.frame_id = frame_id_.empty() ? "world" : frame_id_;
  empty_cloud.header.stamp = ros::Time::now();
  empty_cloud.height = 1;
  empty_cloud.width = 0;
  empty_cloud.is_dense = true;

  // Publish empty clouds to ALL visualization topics (don't check subscribers)
  if (occupied_pub_) occupied_pub_.publish(empty_cloud);
  if (occupied_inflate_pub_) occupied_inflate_pub_.publish(empty_cloud);
  if (unknown_pub_) unknown_pub_.publish(empty_cloud);
  if (free_pub_) free_pub_.publish(empty_cloud);
  if (esdf_pub_) esdf_pub_.publish(empty_cloud);
  if (object_grid_pub_) object_grid_pub_.publish(empty_cloud);
  if (value_map_pub_) value_map_pub_.publish(empty_cloud);
  if (depth_cloud_pub_) depth_cloud_pub_.publish(empty_cloud);
  if (filtered_depth_cloud_pub_) filtered_depth_cloud_pub_.publish(empty_cloud);
  if (all_object_cloud_pub_) all_object_cloud_pub_.publish(empty_cloud);
  if (filtered_object_cloud_pub_) filtered_object_cloud_pub_.publish(empty_cloud);
  if (over_depth_object_cloud_pub_) over_depth_object_cloud_pub_.publish(empty_cloud);
  if (confidence_map_pub_) confidence_map_pub_.publish(empty_cloud);

  // update_range is a Marker, needs DELETEALL instead of empty cloud.
  if (update_range_pub_) {
    visualization_msgs::Marker del;
    del.header.frame_id = empty_cloud.header.frame_id;
    del.header.stamp = empty_cloud.header.stamp;
    del.ns = "update_range";
    del.id = 0;
    del.action = visualization_msgs::Marker::DELETEALL;
    update_range_pub_.publish(del);
  }

  ROS_INFO("[MapROS] Map visualization cleared");
}
...
void MapROS::init()
{
  ...
  // alive_token_ must be initialised before any subscriber so the weak_ptr
  // captured by these callbacks is meaningful from the first message.
  alive_token_ = std::make_shared<int>(1);
  std::weak_ptr<int> weak_alive_pre = alive_token_;
  detected_object_cloud_sub_ = node_.subscribe<plan_env::MultipleMasksWithConfidence>(
      "/detector/clouds_with_scores", 10,
      boost::function<void(const plan_env::MultipleMasksWithConfidenceConstPtr&)>(
        [this, weak_alive_pre](const plan_env::MultipleMasksWithConfidenceConstPtr& msg) {
          if (weak_alive_pre.expired()) return;
          detectedObjectCloudCallback(msg);
        }));
  itm_score_sub_ = node_.subscribe<std_msgs::Float64>(
      "/blip2/cosine_score", 10,
      boost::function<void(const std_msgs::Float64::ConstPtr&)>(
        [this, weak_alive_pre](const std_msgs::Float64::ConstPtr& msg) {
          if (weak_alive_pre.expired()) return;
          itmScoreCallback(msg);
        }));
  itm_scores_multi_sub_ = node_.subscribe<std_msgs::Float64MultiArray>(
      "/blip2_itm/scores", 10,
      boost::function<void(const std_msgs::Float64MultiArray::ConstPtr&)>(
        [this, weak_alive_pre](const std_msgs::Float64MultiArray::ConstPtr& msg) {
          if (weak_alive_pre.expired()) return;
          itmScoresMultiCallback(msg);
        }));
  ...
  // Lifetime-safe callback (alive_token_ was created above with the plain
  // subscribers; same token serves both). When the destructor resets it, any
  // sync output still in CallbackQueue locks the weak_ptr, fails, and skips.
  std::weak_ptr<int> weak_alive = alive_token_;
  sync_image_pose_->registerCallback(
      boost::bind<void>(
        [this, weak_alive](const sensor_msgs::Image::ConstPtr& img,
                           const nav_msgs::Odometry::ConstPtr& odom) {
          if (weak_alive.expired()) return;
          depthPoseCallback(img, odom);
        },
  ...
  itm_score_ = -1.0;
  itm_scores_multi_.assign(2, 0.0);
  has_multi_scores_ = false;
...
{
  // Legacy single-score path. Treated as the SR (target-direct) score
  // when the multi-array topic has not yet arrived.
  if (!has_multi_scores_) {
    itm_score_ = msg->data;
  }
}

void MapROS::itmScoresMultiCallback(const std_msgs::Float64MultiArrayConstPtr& msg)
{
  if (msg->data.size() < 2) {
    ROS_WARN_THROTTLE(5.0,
        "[MapROS] /blip2_itm/scores expected >=2 values, got %zu", msg->data.size());
    return;
  }
  itm_scores_multi_[0] = msg->data[0];  // SR
  itm_scores_multi_[1] = msg->data[1];  // IG
  has_multi_scores_ = true;
  // SR drives ObjectMap and legacy value_map_; expose it on itm_score_ for downstream code.
  itm_score_ = itm_scores_multi_[0];
}
...
{
  // Belt-and-braces lifetime guard. message_filters can dispatch a sync output
  // to this callback even after the underlying Subscriber has been destroyed
  // (ROS Subscription queue holds the bound callable independently). Both
  // checks must hold or we are dispatching on freed memory.
  if (!alive_token_) return;
  ...
  t1 = ros::Time::now();
  // Update legacy semantic value map (uses SR/target ITM score)
  if (itm_score_ != -1.0)
    map_->value_map_->updateValueMap(camera_pos, camera_yaw, free_grids, itm_score_);

  // Update two-map fusion manager when multi-prompt ITM scores are available
  if (mvm_ != nullptr && has_multi_scores_) {
    mvm_->updateAllValueMaps(camera_pos, camera_yaw, free_grids, itm_scores_multi_);
  }
