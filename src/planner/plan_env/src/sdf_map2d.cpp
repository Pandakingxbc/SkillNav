...
// Match ApexNav: default destruction (members destruct in reverse decl order).
...
void SDFMap2D::initMap(ros::NodeHandle& nh)
{
  ...
  // Initialize Voronoi topology (decoupled, runs independently)
  bool enable_voronoi_topology;
  nh.param("voronoi_topology/enable", enable_voronoi_topology, false);
  if (enable_voronoi_topology) {
    voronoi_topology_.reset(new VoronoiTopology());
    // Use shared_from_this pattern - need to pass this as shared_ptr
    // For now, create a temporary shared_ptr wrapper
    auto sdf_map_ptr = std::shared_ptr<SDFMap2D>(this, [](SDFMap2D*){});
    voronoi_topology_->init(nh, sdf_map_ptr);
    ROS_INFO("[SDFMap2D] Voronoi topology enabled");
  }
}

void SDFMap2D::getUpdateBoundIndices(Eigen::Vector2i& bmin, Eigen::Vector2i& bmax) const {
  bmin = md_->update_min_;
  bmax = md_->update_max_;
}

bool SDFMap2D::isFreeCell(const Eigen::Vector2i& idx) const {
  if (idx(0) < 0 || idx(0) >= mp_->map_voxel_num_(0) ||
      idx(1) < 0 || idx(1) >= mp_->map_voxel_num_(1)) {
    return false;
  }
  int adr = idx(0) * mp_->map_voxel_num_(1) + idx(1);
  if (md_->occupancy_buffer_inflate_[adr] == 1) return false;                  // inflated obstacle
  if (md_->occupancy_buffer_[adr] < mp_->clamp_min_log_ - 1e-3) return false;  // unknown
  if (md_->occupancy_buffer_[adr] > mp_->min_occupancy_log_) return false;     // occupied
  return true;
}
...
double SDFMap2D::getExploredFraction() const
{
  // Mirror the UNKNOWN test from getOccupancy(): a cell is "explored" once
  // its log-odds has been bumped above the unknown sentinel by at least one
  // raycast update. Walking the raw buffer keeps this O(N) without virtual
  // calls; at ~3 s cadence it is negligible relative to one LLM round-trip.
  if (md_->occupancy_buffer_.empty()) return 0.0;
  const double unknown_threshold = mp_->clamp_min_log_ - 1e-3;
  size_t observed = 0;
  for (double v : md_->occupancy_buffer_) {
    if (v >= unknown_threshold) ++observed;
  }
  return static_cast<double>(observed) /
         static_cast<double>(md_->occupancy_buffer_.size());
}
