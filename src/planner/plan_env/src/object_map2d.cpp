...
ObjectMap2D::~ObjectMap2D() {
  // Clear /object/clouds so stale detections don't bleed into the next episode.
  if (ros::ok() && object_cloud_pub_) {
    ...
    empty.width = 0;
    empty.is_dense = true;
    object_cloud_pub_.publish(empty);
...
bool ObjectMap2D::adjustClusterConfidence(int cluster_id, double adjustment)
{
  if (cluster_id < 0 || cluster_id >= static_cast<int>(objects_.size())) {
    ROS_WARN("[ObjectMap2D] Invalid cluster_id %d for confidence adjustment", cluster_id);
    return false;
  }

  auto& obj = objects_[cluster_id];
  int label = obj.best_label_;
  if (label < 0 || label >= static_cast<int>(obj.confidence_scores_.size())) {
    ROS_WARN("[ObjectMap2D] Invalid label %d for cluster %d", label, cluster_id);
    return false;
  }

  double old_conf = obj.confidence_scores_[label];
  double new_conf = old_conf * adjustment;

  // Clamp to valid range [0, 1]
  new_conf = std::max(0.0, std::min(1.0, new_conf));

  obj.confidence_scores_[label] = new_conf;

  ROS_INFO("[ObjectMap2D] Adjusted cluster %d confidence: %.3f -> %.3f (x%.2f)",
           cluster_id, old_conf, new_conf, adjustment);

  return true;
}

double ObjectMap2D::getClusterConfidence(int cluster_id) const
{
  if (cluster_id < 0 || cluster_id >= static_cast<int>(objects_.size())) {
    return -1.0;
  }

  const auto& obj = objects_[cluster_id];
  int label = obj.best_label_;
  if (label < 0 || label >= static_cast<int>(obj.confidence_scores_.size())) {
    return -1.0;
  }

  return obj.confidence_scores_[label];
}
