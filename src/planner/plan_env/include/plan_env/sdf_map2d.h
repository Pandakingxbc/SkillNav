...
public:
  ...
  /// Fraction of in-bounds cells that have been observed (i.e. not UNKNOWN).
  /// O(N) over the occupancy buffer — fine for low-rate consumers like the
  /// strategic agent (~3 s cadence). Returns 0.0 before the first sensor
  /// update arrives.
  ...
  shared_ptr<VoronoiTopology> voronoi_topology_;  ///< Voronoi-based topology for navigation
...
public:
  ...
  /// Access the embedded MapROS instance (e.g., to wire MultiValueMapManager).
  MapROS* getMapROS() const { return map_ros_.get(); }

  /// Cumulative bounds of cells touched since startup. Useful for full-map visualization.
  void getUpdateBoundIndices(Eigen::Vector2i& bmin, Eigen::Vector2i& bmax) const;

  /// Returns true if cell is observed AND classified as free space.
  bool isFreeCell(const Eigen::Vector2i& idx) const;
};
