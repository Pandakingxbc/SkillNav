...
public:
  ...
  /**
   * @brief Adjust confidence score of a specific object cluster
   * Used by Memory Agent for VLM verification feedback
   *
   * @param cluster_id Object cluster ID
   * @param adjustment Multiplier: <1.0 reduces, 1.0 no change, >1.0 boosts
   * @return true if cluster found and updated
   */
  bool adjustClusterConfidence(int cluster_id, double adjustment);

  /**
   * @brief Get confidence score of a specific cluster
   * @param cluster_id Object cluster ID
   * @return Confidence score, or -1 if cluster not found
   */
  double getClusterConfidence(int cluster_id) const;
