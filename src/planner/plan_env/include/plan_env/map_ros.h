...
public:
  ...
  /// Attach MultiValueMapManager to enable two-map fusion data flow.
  /// Safe to call after MVMM is constructed (typically by ExplorationManager).
  void setMultiValueMapManager(MultiValueMapManager* mvm) { mvm_ = mvm; }

  /// Clear all visualization by publishing empty point clouds
...
private:
  ...
  void itmScoresMultiCallback(const std_msgs::Float64MultiArrayConstPtr& msg);
  ...
  ros::Subscriber detected_object_cloud_sub_, itm_score_sub_, itm_scores_multi_sub_;
  ...
  double itm_score_;               ///< Current target-direct (SR) ITM score; -1 if none received yet
  std::vector<double> itm_scores_multi_;  ///< Latest [SR, IG] scores from /blip2_itm/scores
  bool has_multi_scores_;          ///< True once at least one multi-array message arrived
  MultiValueMapManager* mvm_ = nullptr;   ///< Optional fusion manager (set by ExplorationManager)

  // Lifetime guard for message_filters sync callbacks: bound by weak_ptr into
  // the registered callback. Destructor resets this so any sync output still
  // queued in CallbackQueue after MapROS dies is safely skipped instead of
  // dispatching depthPoseCallback on freed memory.
